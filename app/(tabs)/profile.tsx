...
// NEW: Import your computeImpact function
import { computeImpact } from '../../utils/points';

export default function ProfileScreen(){
  const { user, communities } = useStore();

  // Find all posts made by the current user
  const myPosts = communities.flatMap(c => c.posts.filter(p => p.userId === user.id).map(p => ({...p, community:c.name})));

  // NEW: Calculate the total impact score by summing up the impact of each post
  const totalImpact = myPosts.reduce((acc, post) => {
    return acc + computeImpact({ up: post.up, down: post.down });
  }, 0);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        {/* CHANGED: Display the newly calculated totalImpact */}
        <Text style={styles.kpi}>Impact Points: <Text style={{color:'#08313B', fontWeight:'800'}}>{totalImpact}</Text></Text>
